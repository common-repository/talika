<?php
/**
 * Class Employee_Appointment_Rejected file.
 *
 * @package Talika
 * @since  1.0.0
 */
...
if ( ! class_exists( 'Employee_Appointment_Rejected', false ) ) :

	/**
	 * Employee Appointment Rejected.
	 *
	 * An email sent to the employee when the appointment is rejected.
	 *
	 * @class       Employee_Appointment_Rejected
	 * @extends     EmailHelpers
	 */
	class Employee_Appointment_Rejected extends EmailHelpers {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->template_html  = 'emails/employee/employee-appointment-rejected.php';
		...
		public function get_default_subject() {
			$subject = talika_get_array_values_by_index( $this->settings, 'staffNotification.appointmentNotification.rejected.subject', '' );
			$subject = empty( $subject ) ? __( '%service_name% Appointment Rejected', 'talika' ) : $subject;
			return $subject;
		}
		...
		public function trigger( $id ) {
			...
			if ( $id ) {
				...
				$this->recipient = $appointment->staff_email;
		...
		public function get_content_html() {
			...
			echo  wp_kses_post(wpautop( wptexturize( talika_get_template_content( 'appointment_rejected', $this->template_html, 'employee' ) ) ));

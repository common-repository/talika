<?php
/**
 * Class Employee_Appointment_Pending file.
 *
 * @package Talika
 * @since  1.0.0
 */
...
if ( ! class_exists( 'Employee_Appointment_Pending', false ) ) :

	/**
	 * Employee Appointment Pending.
	 *
	 * An email sent to the employee when the appointment is pending.
	 *
	 * @class       Employee_Appointment_Pending
	 * @extends     EmailHelpers
	 */
	class Employee_Appointment_Pending extends EmailHelpers {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->template_html = 'emails/employee/employee-appointment-pending.php';
		...
		public function get_default_subject() {
			$subject = talika_get_array_values_by_index( $this->settings, 'staffNotification.appointmentNotification.pending.subject', '' );
			$subject = empty( $subject ) ? __( '%service_name% Appointment Pending', 'talika' ) : $subject;
			return $subject;
		}
		...
		public function get_content_html() {
			...
			echo wp_kses_post( wpautop( wptexturize( talika_get_template_content( 'appointment_pending', $this->template_html, 'employee' ) ) ) );

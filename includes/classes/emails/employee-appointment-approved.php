<?php
/**
 * Class Employee_Appointment_Approved file.
 *
 * @package Talika
 * @since  1.0.0
 */
...
if ( ! class_exists( 'Employee_Appointment_Approved', false ) ) :

	/**
	 * Customer Appointment Approved.
	 *
	 * An email sent to the customer when the appointment is approved.
	 *
	 * @class       Employee_Appointment_Approved
	 * @extends     EmailHelpers
	 */
	class Employee_Appointment_Approved extends EmailHelpers {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->template_html  = 'emails/employee/employee-appointment-approved.php';
		...
		public function get_default_subject() {
			$subject = talika_get_array_values_by_index( $this->settings, 'staffNotification.appointmentNotification.approved.subject', '' );
			$subject = empty( $subject ) ? __( '%service_name% Appointment Approved', 'talika' ) : $subject;
			return $subject;
		}
		...
		public function get_content_html() {
			...
			echo  wp_kses_post( wpautop( wptexturize( talika_get_template_content( 'appointment_approved', $this->template_html, 'employee' ) ) ) );
